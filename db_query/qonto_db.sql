...
-- products.category (в UI уже есть, но в схеме отсутствовал)
ALTER TABLE `products`
  ADD COLUMN IF NOT EXISTS `category` VARCHAR(100) NOT NULL DEFAULT '' AFTER `description`;

-- превью-картинка для каталога
ALTER TABLE `products`
  ADD COLUMN IF NOT EXISTS `preview_image_url` VARCHAR(500) NULL DEFAULT NULL
  AFTER `status`;