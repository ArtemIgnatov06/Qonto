...
ALTER TABLE products
ADD COLUMN preview_image_url VARCHAR(500) NULL DEFAULT NULL
AFTER status;
...
INSERT IGNORE INTO chat_threads (seller_id, buyer_id)
VALUES (?, ?);
...
UPDATE chat_threads
SET muted_unread_seller = muted_unread_seller + ?
WHERE id = ?;
-- name: update_general_06
UPDATE chat_threads
SET muted_unread_buyer = muted_unread_buyer + ?
WHERE id = ?;
...
INSERT INTO order_addresses (order_id, country, city, street, postal_code)
VALUES (?, ?, ?, ?, ?);
...
INSERT INTO payments (order_id, provider, brand, last4, status)
